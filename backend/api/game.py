...
def calculate_accuracy(all_games):
    prediction_is_accurate = 0
    prediction_is_inaccurate = 0
...
def api_get_backtest(request):
    ...
    for game in games:
        ...
        if date_string in all_games:
            cur = all_games[date_string]
            cur.append(game_detail)
            all_games[date_string] = cur
        else:
            all_games[date_string] = [game_detail]
